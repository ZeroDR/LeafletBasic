    ...
    request(params, callback1, callback0) {
        ...
        Axios({
            ...
            if (!result.data.errorcode) {
                ...
                let hasSuccess = parseInt(data.Status || data.status);
                if (hasSuccess > 0) {
                    data = {
                        status: 1,
                        ...
                        obj: (typeof data.obj === 'string' ? JSON.parse(data.obj) : data.obj) || (typeof data.Data === 'string' ? JSON.parse(data.Data) : data.Data) || (typeof data.Data === 'string' ? JSON.parse(data.case) : data.case)
                    };
                    callback1(data);
                }
                else {
